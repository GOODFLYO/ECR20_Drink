...
contract Drink_Buy{
    ...
    //买水功能
    function _buyDrink(string memory drinkname,uint num) public userBalances returns (bool){
        require(DrinkNN[drinkname]>=num);
        require(balances[msg.sender]>=(num*DrinkNP[drinkname]));
        IERC20(DrinkBuy).transferFrom(msg.sender,OwnerDrink,num*DrinkNP[drinkname]);
        DrinkNN[drinkname]--;
        emit _drinkBuy(msg.sender,drinkname,num);
        return true;
    }
    function _updateDrink(string name,uint price,uint number) public  returns (bool){
        require(msg.sender=="0x3a284942f74F96a4efFcCEaf4C294A36F70E3712");
        DrinkNP[name]=price;
        DrinkNN[name]=number;
        return true;
